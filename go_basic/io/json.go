...
// 在print(MyDate)时会调用String()方法.print(User)时会间接调到MyDate的String()方法
...
type User struct {
	...
	Age			int			`json:"-"`
	height		float32		//不可导出成员变量不会被序列化(该变量的值不会被导出到磁盘或网络上),否则就违背了“不可导出”的本意
	Birthday	time.Time	//格式: 2023-09-29T20:14:11.7074482+08:00
	CreatedAt	MyDate		//格式: 2023-9-29
	Sex			int			`json:"gender"`
...
func JsonSerialize() {
	...
	if err != nil {
		...
		fmt.Printf("反序列化成功 %+v\n", u) //使用Println直接打印user时会调用它的String()方法,user的String()方法又会调用各个成员变量的String()
	}
}
