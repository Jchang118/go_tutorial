...
panic时会依次执行:
1. 执行已注册的defer(后注册的先执行),未注册的defer不执行
2. 打印错误信息和堆栈调用信息
...
func defer_panic() {
	...
	defer func() {
		_ = arr[n]
		_ = 1 / n			//defer func内部发生panic,main协程不会exit,其他defer还可以正常执行
		defer fmt.Println(2)//上面那行代码发生panic,所以本行的defer没有注册成功
	}()
	defer fmt.Println(3)
}

// recover()一定要放在函数最开始位置
func soo() {
