...
/*
Go函数可以是一个闭包.闭包是一个函数值,它引用了函数体之外的变量.这个函数可以对这个引用的变量进行访问和赋值;换句话说这个函数被“绑定”在这个变量上.
*/
...
func funcTimer() {
	...
	defer func() { //defer后面跟闭包.闭包引用了begin变量
...
func forRange() {
	...
	wg.Add(len(arr))
	for _, v := range arr {
		go func() { //go后面跟闭包.闭包里引用了v变量(每一轮for循环会创建全新的v),通常进入下轮循环时上一轮的v变量就不可访问了,但由于闭包,它仍被闭包函数持有
			defer wg.Done()
			time.Sleep(time.Second)
			fmt.Printf("%d\t", v) //输出 1 2 3 4 (随机顺序)
		}()
	}
	wg.Wait()
	fmt.Println()
