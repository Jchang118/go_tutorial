...
func switch_basic() {
	...
	case "green": //case后面跟一个常量,只要switch和case后面的值类型相同就行
		fmt.Println("go")
	case "red":
		fmt.Println("stop")
	case "yellow":
...
func swtich_condition() {
	color := "yellow"
	switch color {
	case "green":
		fmt.Println("go")
	case "red", "yellow": //用逗号分隔多个condition,它们之间是“或”的关系,只需要一个condition满足就行
		fmt.Println("stop")
	}
...
// fallthrough强制执行下一个case(或default)
...
func switch_type() {
	...
	case int: //value已经被转换为int类型
		fmt.Printf("number is int %d\n", value)
	case float64: //value已经被转换为float64类型
		fmt.Printf("number is float64 %f\n", value)
	case byte, string:
		fmt.Printf("number is interface %v\n", value)
	default:
		fmt.Println("neither")
	}
