...
// zero value,零值,默认值
func main19() {
	var i int //各种整形(包括rune), 0
	fmt.Printf("i=%v\n", i)
	var f float32 // 各种浮点数,0
	fmt.Printf("f=%v\n", f)
	var b byte //0
	fmt.Printf("b=%v\n", b)
	var bl bool //false
	...
	var u User //结构体的每一个成员变量分别为相对应数据类型的“0”值,可能会存在嵌套
	fmt.Printf("u=%v\n", u)
	...
	fmt.Printf("mp=%v\n", mp)	//长度为0的空map
	// mp[2] = true		//panic: assignement to entry in nil map. map必须经过初始化才能进行写
	var ch chan int //引用类型,nil
	fmt.Printf("ch=%v\n", ch)
	// var fk func(bool) error //函数类型nil
	// fmt.Printf("fk=%v\n", fk)
