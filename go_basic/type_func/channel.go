...
func rangeChannel() {
	...
	ch = make(chan int, 8) //初始化,环形队列可容纳8个int
	ch <- 1				   //往管道里写入(send)数据
	ch <- 2
	ch <- 3
	ch <- 4 
	...
	close(ch)
	//遍历并取走(receive)管道里的元素.当管道里已无剩余元素并没有close管道时,receive操作会一直阻塞,最终报deadlock.当管道为空且被close后,for循环退出
	for ele := range ch {
		fmt.Println(ele)
	}
...
// slice、map、channel是go语言里的三大引用类型,如果只想改变它们引用的底层数据,不需要传指针,因为传引用类型本质上传的就是底层数据的指针
