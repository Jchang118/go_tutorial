...
import (
    "fmt"
    grpc_service "go_tutorial/go_basic/grpc/idl/service"
    "context"
    "time"
    "log"
    "io"
...
func streaming(client grpc_service.StudentClient) {
    ...
    stream4.CloseSend() //关闭流.客户端创建的stream最终都要调close,server端不用调close.正如http client需要close response body,而http server不需要 close request body
    <-done              //done之后往下走
    fmt.Println()
}
