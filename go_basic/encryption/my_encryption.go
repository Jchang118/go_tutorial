...
type MyEncription struct {
    key         [8]byte
    BlockSize   int
    BlockMode   int
}

func NewMyEncryption(key [8]byte, BlockMode int) *MyEncription {
    return &MyEncription{
        key:        key,
        BlockSize:  8,
        BlockMode:  BlockMode,
...
func (en *MyEncription) encryptBlock(plain, cypher []byte) {
...
func (en *MyEncription) decryptBlock(plain, cypher []byte) {
    for j := 0; j < len(plain); j++ {
        plain[j] = cypher[j] ^ en.key[j] //不开辟额外的内存空间,就地加密
    }
}

// 分组混淆.plain和prevCypher的长度一定相等
func (en *MyEncription) confuseBlock(plain, prevCypher []byte) {
    switch en.BlockMode {
    case CBC:
        for j := 0; j < len(plain); j++ {
...
func (en *MyEncription) deconfuseBlock(plain, prevCypher []byte) {
    en.confuseBlock(plain, prevCypher)
}

func (en *MyEncription) Encrypt(plain []byte) []byte {
    plainPadding := PKCS7.Padding(plain, en.BlockSize) //明文末尾填充字节,长度为BlockSize的整倍数
...
func (en *MyEncription) Decrypt(cypher []byte) ([]byte, error) {
