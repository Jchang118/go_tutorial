...
func BenchmarkLoggerSlog(b *testing.B) {
	...
	b.ResetTimer()
	for b.Loop() {
		logger.Info(LOG, "name", "joseph", "age", 18)
		// logger.Info(LOG, slog.String("name", "joseph"), slog.Int("age", 18)) //显式指定数据类型也没有带来性能的提升
	}
}
...
func BenchmarkLoggerLogrus(b *testing.B) {
	...
	b.ResetTimer()
	for b.Loop() {
		logger.WithFields(logrus.Fields{"name": "joseph", "age": 18}).Info(LOG)
	}
}
